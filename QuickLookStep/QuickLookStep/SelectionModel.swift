...
struct SelectionModel {
    ...
    init?(geometry: SCNGeometry, settings: SelectionModelSettings = .init()) {
        guard let mesh = SelectionGeometryReader.readMesh(from: geometry) else {
            return nil
        }
    ...
    private static func populateTopology(
    ...
    private static func populateWeldedFeatureFlags(
        edges: inout [SelectionEdge],
    ...
    private static func weldedEdgeKey(
    ...
    private static func weldedVertexTolerance(maxExtent: Float, settings: SelectionModelSettings) -> Float {
