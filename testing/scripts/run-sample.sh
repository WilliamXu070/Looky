...
SAMPLE="${1:-$ROOT/testing/input/cube_hole.step}"
OUTPUT="${2:-$ROOT/testing/results/sample-quickrun-$(date +%Y%m%d-%H%M%S).json}"

if [[ ! -f "$SAMPLE" ]]; then
  echo "Sample STEP not found: $SAMPLE"
  exit 1
fi
...
if [[ -z "$APP" ]]; then
  echo "QuickLookStep binary not found in build products."
...
PLAN_FILE="$(mktemp "$ROOT/testing/plans/.sample-run.XXXXXX")"
...
cat > "$PLAN_FILE" <<JSON
{
  "scenarios": [
    {
      "name": "sample-quickrun",
      "file": "$SAMPLE",
      "actions": [
        { "kind": "rotateY", "value": 15, "durationMs": 140 },
        { "kind": "rotateX", "value": -10, "durationMs": 120 },
...
export QLS_TEST_PLAN="$PLAN_FILE"
...
if ! launch; then
  echo "Unable to start QuickLookStep."
