...
### Problem context
- Sidecar-only SolidWorks support still required users to manually export a mesh/CAD file beside the `.SLDPRT`; the viewer needed a local conversion hook for machines that have a SolidWorks-capable converter installed.

### What changed
- Added the `QLS_SOLIDWORKS_CONVERTER` contract: set it to an executable path, and `SceneBuilder` calls it as `<converter> <input.sldprt> <output-dir>`. The converter must write or print a supported output path (`step/stp/3mf/glb/gltf/obj/stl`).
- Converter output is cached under `~/Library/Caches/QuickLookStep/SolidWorksConversions/<fingerprint>/` and then loaded through the normal QuickLookStep scene path.

### Why it helped
- Keeps SolidWorks conversion local and tool-agnostic while preventing JPG preview fallback; any real local converter can now plug into QuickLookStep without changing app code.

### Validation
- `QLS_SOLIDWORKS_CONVERTER=/Users/williamxu/Desktop/Projects/quicklook/testing/solidworks/scripts/fake_solidworks_converter.sh /Users/williamxu/Desktop/Projects/quicklook/build/Build/Products/Debug/QuickLookStep.app/Contents/MacOS/QuickLookStep --sample "/Users/williamxu/Downloads/spur-gear-415.snapshot.1/Gear.SLDPRT"`
- Check logs for `SolidWorks converter produced ... Gear.obj` and `Loaded ... via SolidWorks local converter output`.
- `QLS_SOLIDWORKS_CONVERTER=/Users/williamxu/Desktop/Projects/quicklook/testing/solidworks/scripts/fake_solidworks_converter.sh testing/scripts/verify_quicklook_ui_launch.sh /Users/williamxu/Desktop/Projects/quicklook/build/Build/Products/Debug/QuickLookStep.app "/Users/williamxu/Downloads/spur-gear-415.snapshot.1/Gear.SLDPRT" /tmp/quicklook-ui-launch-check-gear-sldprt-converter.png`

## 2026-07-04 Update

### Problem context
- `Gear.SLDPRT` still showed the SolidWorks converter error when opened through the normal macOS `open`/Finder path because GUI-launched apps do not inherit shell-only environment variables and QuickLookStep reads sandbox-container preferences.

### What changed
- Documented that `SceneBuilder` reads the local converter from either `QLS_SOLIDWORKS_CONVERTER` env or persistent app defaults keys `QLS_SOLIDWORKS_CONVERTER` / `solidWorksConverterPath`.
- For Finder/QuickLook-style validation, configure the converter in the app container preferences before launching.

### Why it helped
- Separates a missing converter configuration from a broken loader and makes the same `.SLDPRT` path work in both direct CLI and GUI launch flows once a real converter is installed.

### Validation
- `defaults write "$HOME/Library/Containers/com.johnboiles.QuickLookStep/Data/Library/Preferences/com.johnboiles.QuickLookStep" QLS_SOLIDWORKS_CONVERTER "/Users/williamxu/Desktop/Projects/quicklook/testing/solidworks/scripts/fake_solidworks_converter.sh"`
- `killall cfprefsd`
- `open -n -a "/Users/williamxu/Desktop/Projects/quicklook/build/Build/Products/Debug/QuickLookStep.app" --args --sample "/Users/williamxu/Downloads/spur-gear-415.snapshot.1/Gear.SLDPRT"`
- Check logs for `Using SolidWorks converter from defaults` and `Loaded ... via SolidWorks local converter output`.
