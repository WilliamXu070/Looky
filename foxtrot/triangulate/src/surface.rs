...
use crate::{
    Error,
    mesh::{SurfaceDescriptor, SurfaceKind, Vertex},
};
    ...
    pub fn descriptor(&self) -> SurfaceDescriptor {
        match self {
            Surface::Plane { location, normal, .. } => {
                let mut descriptor = SurfaceDescriptor::new(SurfaceKind::Plane);
                descriptor.origin = *location;
                descriptor.normal = *normal;
                descriptor
            }
            Surface::Cylinder { location, axis, radius, .. } => {
                let mut descriptor = SurfaceDescriptor::new(SurfaceKind::Cylinder);
                descriptor.origin = *location;
                descriptor.axis = *axis;
                descriptor.radius = *radius;
                descriptor
            }
            Surface::Cone { location, axis, angle, .. } => {
                let mut descriptor = SurfaceDescriptor::new(SurfaceKind::Cone);
                descriptor.origin = *location;
                descriptor.axis = *axis;
                descriptor.half_angle = *angle;
                descriptor
            }
            Surface::Sphere { location, radius, .. } => {
                let mut descriptor = SurfaceDescriptor::new(SurfaceKind::Sphere);
                descriptor.origin = *location;
                descriptor.radius = *radius;
                descriptor
            }
            Surface::Torus { location, axis, major_radius, minor_radius, .. } => {
                let mut descriptor = SurfaceDescriptor::new(SurfaceKind::Torus);
                descriptor.origin = *location;
                descriptor.axis = *axis;
                descriptor.radius = *major_radius;
                descriptor.secondary_radius = *minor_radius;
                descriptor
            }
            Surface::BSpline(_) | Surface::NURBS(_) => {
                SurfaceDescriptor::new(SurfaceKind::BSpline)
            }
        }
    }
