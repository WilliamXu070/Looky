...
use crate::{
    Error,
    curve::Curve,
    mesh, mesh::{CurveKind, Mesh, SourceFace, SurfaceDescriptor, SurfaceKind, Triangle},
    stats::Stats,
    surface::Surface
};
    ...
    let mesh_fold = to_mesh_iter
        .fold(
            ...
            |(mut mesh, mut stats), (id, mats)| {
                ...
                let face_start = mesh.source_faces.len();
                let edge_start = mesh.source_edges.len();
                match &s[*id] {
                ...
                let face_end = mesh.source_faces.len();
                let edge_end = mesh.source_edges.len();
                let brep_id = id.0 as u64;
                for face in &mut mesh.source_faces[face_start..face_end] {
                    face.brep_id = brep_id;
                }
                for edge in &mut mesh.source_edges[edge_start..edge_end] {
                    edge.brep_id = brep_id;
                }
                let base_faces = mesh.source_faces[face_start..face_end].to_vec();
                let base_edges = mesh.source_edges[edge_start..edge_end].to_vec();

                for (instance_index, mat) in mats.iter().enumerate().skip(1) {
                    ...
                    let triangle_copy_start = mesh.triangles.len();
                    for t in t_start..t_end {
                        let mut tri = mesh.triangles[t];
                        tri.verts.add_scalar_mut(offset as u32);
                        mesh.triangles.push(tri);
                    }
                    mesh.source_faces.extend(base_faces.iter().cloned().map(|mut face| {
                        face.instance_id = instance_index as u64;
                        face.triangle_start = triangle_copy_start + (face.triangle_start - t_start);
                        face.surface = transform_surface_descriptor(&face.surface, mat);
                        face
                    }));
                    mesh.source_edges.extend(base_edges.iter().cloned().map(|mut edge| {
                        edge.instance_id = instance_index as u64;
                        edge.points = edge.points.iter().map(|point| transform_point(mat, point)).collect();
                        edge
                    }));
                }
...
fn transform_point(mat: &DMat4, point: &DVec3) -> DVec3 {
    (mat * DVec4::new(point.x, point.y, point.z, 1.0)).xyz()
}

fn transform_direction(mat: &DMat4, direction: &DVec3) -> DVec3 {
    let transformed = (mat * DVec4::new(direction.x, direction.y, direction.z, 0.0)).xyz();
    if transformed.norm_squared() > 0.0 {
...
fn transform_surface_descriptor(
    descriptor: &SurfaceDescriptor,
    mat: &DMat4,
) -> SurfaceDescriptor {
    let mut transformed = descriptor.clone();
    match descriptor.kind {
        SurfaceKind::Plane => {
            transformed.origin = transform_point(mat, &descriptor.origin);
            transformed.normal = transform_direction(mat, &descriptor.normal);
        }
        SurfaceKind::Cylinder | SurfaceKind::Cone | SurfaceKind::Torus => {
            transformed.origin = transform_point(mat, &descriptor.origin);
            transformed.axis = transform_direction(mat, &descriptor.axis);
        }
        SurfaceKind::Sphere => {
            transformed.origin = transform_point(mat, &descriptor.origin);
        }
        SurfaceKind::BSpline | SurfaceKind::Other => {}
    }
    transformed
}
                 ...
                 stats: &mut Stats) -> Result<(), Error>
{
    ...
    for b in &face.bounds {
        let bound = face_bound(s, *b)?;
        for edge in bound.edges {
            mesh.record_source_edge(edge.entity_id, face_id, edge.kind, edge.points);
        }
        let bound_contours = bound.points;
    ...
    let triangle_count = mesh.triangles.len() - triangle_start;
    if triangle_count > 0 {
        mesh.source_faces.push(SourceFace {
            brep_id: 0,
            instance_id: 0,
            entity_id: face_id,
            triangle_start,
...
struct FaceBoundSample {
    points: Vec<DVec3>,
    edges: Vec<EdgeSample>,
}

fn face_bound(s: &StepFile, b: FaceBound) -> Result<FaceBoundSample, Error> {
    ...
    match &s[bound] {
        ...
        Entity::VertexLoop(v) => {
            ...
            Ok(FaceBoundSample {
                points: vec![vertex_point(s, v.loop_vertex)],
                edges: Vec::new(),
            })
        }
    ...
    -> Result<FaceBoundSample, Error>
{
    ...
    for (i, e) in edge_list.iter().enumerate() {
        ...
        let sample = edge_curve(s, edge.edge_element.cast(), edge.orientation)?;
        out.extend(sample.points.iter().copied());
        edges.push(sample);
    }
    Ok(FaceBoundSample { points: out, edges })
}

fn edge_curve(s: &StepFile, e: EdgeCurve, orientation: bool) -> Result<EdgeSample, Error> {
    ...
    Ok(EdgeSample {
        entity_id: e.0 as u64,
        kind,
        points: curve.build(u, v),
    })
}
